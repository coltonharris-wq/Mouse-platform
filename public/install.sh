...
# Extract API key from config payload (needed regardless of fast/slow path)
MOONSHOT_KEY=""
if [ -n "$CONFIG_B64" ]; then
...
# Check if config was pre-uploaded by provision route (fast path)
if [ -f "$HOME/.openclaw/openclaw.json" ] && [ -s "$HOME/.openclaw/openclaw.json" ] && \
   [ -f "$HOME/.openclaw/workspace/SOUL.md" ] && [ -s "$HOME/.openclaw/workspace/SOUL.md" ]; then
    echo "[Mouse] Config pre-uploaded — skipping onboard (fast path)"
    openclaw doctor --fix 2>/dev/null || true
else
    echo "[Mouse] No pre-uploaded config — running full onboard (slow path)"
    ...
    # Run onboard — let it create proper config, install skills, set up daemon
...
if [ -n "$CONFIG_B64" ]; then
    # Save raw config for reference (always, even on fast path)
    mkdir -p "$INSTALL_DIR/config"
    echo "$CONFIG_B64" | base64 -d > "$INSTALL_DIR/config/config.json"

    # Write API key file (always, even on fast path)
...
# Only generate workspace files if not already pre-uploaded (slow path)
if [ ! -f "$WORKSPACE/SOUL.md" ] || [ ! -s "$WORKSPACE/SOUL.md" ]; then
    echo "[Mouse] Generating workspace files (slow path)..."
...
SOULPATCH
    ...
    echo "[Mouse] Workspace files already present (fast path) — skipping generation"
fi
