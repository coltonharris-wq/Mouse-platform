...
export default function SignupModal({
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<ProTemplateLight | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalIndustry = selectedTemplate?.industry || industry;
    const finalNiche = selectedTemplate?.niche || niche;
    const proTemplateId = selectedTemplate?.id || null;
    ...
    try {
      ...
      sessionStorage.setItem('signup_industry', finalIndustry);
      sessionStorage.setItem('signup_niche', finalNiche);
      sessionStorage.setItem('signup_business_name', businessName || '');
      if (proTemplateId) {
        sessionStorage.setItem('pro_template_id', proTemplateId);
      }

      // Redirect to onboarding wizard (which then redirects to provisioning)
      window.location.href = `/onboarding?customer_id=${data.customer_id}`;
    } catch {
  ...
  const handleGoogleAuth = () => {
    ...
    sessionStorage.setItem('signup_industry', selectedTemplate?.industry || industry);
    sessionStorage.setItem('signup_niche', selectedTemplate?.niche || niche);
    ...
    if (selectedTemplate?.id) {
      sessionStorage.setItem('pro_template_id', selectedTemplate.id);
    }

    const redirectTo = `${window.location.origin}/onboarding?oauth=google`;
    window.location.href = `${supabaseUrl}/auth/v1/authorize?provider=google&redirect_to=${encodeURIComponent(redirectTo)}`;
  };

  const canProceedStep1 = email && password && password.length >= 6;
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-[#0B1F3B]">
              {step === 1 ? 'Create Your Account' : step === 2 ? 'Select Your Industry' : 'Business Details'}
            </h2>
              ...
              {[1, 2, 3].map((s) => (
                <div
                  ...
                  className={`h-1.5 rounded-full transition-all ${
                    s === step ? 'w-8 bg-[#0F6B6E]' : s < step ? 'w-4 bg-[#0F6B6E]/40' : 'w-4 bg-gray-200'
                  }`}
                />
              ))}
            </div>
            ...
            <>
              ...
              <div className="flex gap-3 mt-6">
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="flex-1 py-3 rounded-xl border border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
                >
                ...
                <button
                  ...
                  className="flex-1 bg-[#0F6B6E] text-white py-3 rounded-xl font-semibold hover:bg-[#0B5456] transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  Next
                  <ArrowRight className="w-4 h-4" />
                </button>
          ...
          {step === 3 && (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {selectedTemplate && (
                <div className="bg-[#0F6B6E]/5 border border-[#0F6B6E]/20 rounded-xl p-3">
                  <p className="text-sm text-gray-700">
                    <span className="font-medium">Industry:</span>{' '}
                    {selectedTemplate.emoji} {selectedTemplate.display_name}
                  </p>
                </div>
              )}
