...
CREATE OR REPLACE FUNCTION deduct_hours(p_customer_id UUID, p_hours NUMERIC)
RETURNS VOID AS $$
BEGIN
...
CREATE POLICY IF NOT EXISTS "service_role_provision_status" ON provision_status
  FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "service_role_usage_log" ON usage_log
  FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "service_role_usage_balance" ON usage_balance
  FOR ALL USING (true) WITH CHECK (true);
